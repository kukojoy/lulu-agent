...
def compress_turns(
    ...
    """
    if plan.action != ContextCompressionAction.COMPRESS_NEEDED:
        return None
...
def _render_full_history_source(
    ...
    raw_messages_by_turn = {group.turn_id: group.messages for group in group_messages_by_turn(messages)}
    rendered = []
...
def _render_turns_for_compression(messages: list[Message]) -> str:
...
def _render_messages_for_compression(messages: list[Message]) -> str:
